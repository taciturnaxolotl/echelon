...
export async function createSession(
  ...
  try {
    const initialSlackResponse = await (
      await fetch(
        `https://slack.com/api/oauth.v2.access?code=${code}&client_id=${import.meta.env.SLACK_CLIENT_ID}&client_secret=${import.meta.env.SLACK_CLIENT_SECRET}&redirect_uri=${url}`,
        {
          method: "POST",
          headers: {
    ...
    const session = encodeSession(import.meta.env.JWT_SECRET, partialSession);
...
export function getSession(token: string): Session | null {
  const decodedSession = decodeSession(import.meta.env.JWT_SECRET, token);
